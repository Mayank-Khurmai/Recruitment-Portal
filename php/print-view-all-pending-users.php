<?php
	...
	class admin_preview_view_all_user{
		...
		function __construct(){
			...
			$this->query = "SELECT * FROM login_info WHERE status = 'Pending' ORDER BY student_no";
            $this->response = $this->db->query($this->query);
            $design="<h3 style='text-align:center;text-decoration: underline;'>All Pending Users</h3>";
			if($this->response->num_rows != 0)
			{
                $design .= "<table border='1px' cellpadding='3px' style='width:100%;'>
                           <tr style='background-color:yellow'><th>Sr. No.</th><th>Roll Number</th><th>Email</th><th>Name</th><th>Mobile</th></tr>";
                $y=1;
            ...
            $x->render();
            $x->stream("view-all-pending-users.pdf", array("Attachment" => false));
			exit(0);
